...
# and state files. The next `nimbus-audit` boot will see orphaned
# workers in list-workers and the auditor (or you) can decide whether
# to resume each (`nimbus-worker-resume <slug>`) or cancel them
# (`./scripts/cancel-worker.sh <slug>`).
#
# Scoped to NIMBUS_ROLE=auditor; non-auditor sessions are no-ops.
# The settings.json matcher excludes `clear` and `resume` so /clear
# and --resume don't trigger shutdown.
...
# not have a live working directory anymore; fall back to a sensible
# default if needed.
repo_root=""
...
if [[ -z "$repo_root" || ! -d "$repo_root/.auditor-state" ]]; then
    repo_root="$HOME/Programs/Nimbus"
fi
...
if command -v tmux >/dev/null 2>&1; then
    ...
    tmux kill-session -t nimbus-dashboard 2>/dev/null || true
fi
...
if [[ -d "$state_dir" ]]; then
    ...
    if [[ "$orphaned_count" -gt 0 ]]; then
        osascript -e "display notification \"$orphaned_count worker(s) orphaned by auditor shutdown\" with title \"Nimbus auditor\"" 2>/dev/null || true
    fi
fi
